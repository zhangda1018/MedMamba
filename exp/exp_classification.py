    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, label, padding_mask) in enumerate(train_loader):
                ...
                outputs, adj_list = self.model(batch_x, padding_mask, None, None)
                
                # Classification loss
                loss_cls = criterion(outputs, label.long())
                
                # Structure loss: DAG constraint + sparsity (only for models that return adjacency matrices)
                loss_structure = 0
                if len(adj_list) > 0:
                    # Models like MedGNN return adjacency matrices and need DAG loss
                    lambda_dag = getattr(self.args, 'lambda_dag', 0.5)
                    lambda_sparse = getattr(self.args, 'lambda_sparse', 0.01)
                    for A in adj_list:
                        loss_structure += lambda_dag * dag_loss(A)
                        loss_structure += lambda_sparse * torch.norm(A, p=1)
                # else: MedMamba returns empty list, no structure loss needed
                
                # Total loss
                loss = loss_cls + loss_structure
